...
const useEmergencies = (serviceType: string) => {
  type EmergencyData = {
    emergencyType: string;
    // Add other properties of the emergency data here
  };

  const [emergencies, setEmergencies] = useState<Record<string, EmergencyData>>({});
  const [loading, setLoading] = useState<boolean>(true);
  const [newEmergencyAlert, setNewEmergencyAlert] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const fetchEmergencies = async () => {
      try {
        const response = await axios.get<Record<string, EmergencyData>>(FIREBASE_URL);
        const filteredEmergencies: Record<string, EmergencyData> = {};
